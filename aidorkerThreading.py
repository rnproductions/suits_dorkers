...
search_engines = {
    'Google': '[1](https://www.google.com/search?q=)',
    'Bing': '[2](https://www.bing.com/search?q=)',
    'Startpage': '[3](https://www.startpage.com/do/dsearch?query=)',
    'Yahoo': '[4](https://search.yahoo.com/search?p=)',
    'Yandex': '[5](https://yandex.com/search/?text=)',
...
def fetch_url(query, name, base_url):
    ...
    try:
        ...
        print(f"Error querying {name} with user agent {user_agent}: {e}")

threads = []
for query in queries:
    for name, base_url in search_engines.items():
        t = threading.Thread(target=fetch_url, args=(query, name, base_url))
        threads.append(t)
        t.start()
